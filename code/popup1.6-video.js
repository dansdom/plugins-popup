...
(function($){
	...
	$.fn.popup = function(config)
	{
		...
		var settings = {
							...
							'iframeVideo' : false                 // for AFL matchcentre popup iframe video player
		...
		this.each(function(){
			...
			if (opts.iframeVideo == true)
               {
                   myPopup.videoID = $(this).attr("title");
                   $(this).attr("title","");
                   var myFragment = $(this).attr("name");
                   myPopup.frag = $(myFragment);
               }
               else
               {
                   var myFragment = $(this).attr("name");
                   //myPopup.fragment = $(myFragment);
                   myPopup.frag = myFragment;
               }
	...
	$.fn.popup.createBox = function(opts)
	{
		...
		{
            var transparentLayer = '<div class="transparency" style="z-index:10000;background:#000;opacity:'+(opts.transparentOpacity/100)+';filter:alpha(opacity = '+opts.transparentOpacity+');top:0;left:0;position:absolute"></div>';
        ...
        document.onkeydown = function(e)
		{
			...
			if (keycode == 27)
			{
                   $("#"+opts.popupID).stop().css("display","none");
	...
	$.fn.popup.styleBox = function(popup,opts,properties,image)
	{
		...
		if (opts.centerOnResize === true)
          ...
          if ($(popupSelector).css("display") === "block" && properties && opts.autoSize === true)
          {
              ...
              $(popupSelector+" .galleryControls").css({height:opts.controlHeight+"px","overflow":"hidden"});
		...
		{
              $(popupSelector).css({height:outerBoxHeight,width:outerBoxWidth,"position":"absolute","z-index":10001,"overflow":"hidden"});
              ...
              $(popupSelector+" .galleryTitle").css({height:opts.titleHeight,"overflow":"hidden"});
	...
	$.fn.popup.styleNodeBox = function(popup,opts)
	{
		...
		$("#"+opts.popupID+" ."+opts.contentClass).append('<div class="galleryTitle"><a href="" class="'+opts.closeBox+'">close</a></div>');
          ...
          // steve's addition
          popup.fragment = $(popup.frag);

          // test for video player and playPopupVideo - then append and call function, else just append
          if (opts.iframeVideo === true && typeof playPopupVideo !== 'undefined')
          {
              popup.fragment.appendTo("#"+opts.popupID+" ."+opts.contentClass);
              // start checking to see that the fragment is there, when it is run the play video function
              var videoTimer = setTimeout(function(){$.fn.popup.checkVideo(popup, opts);}, 1000);
          }
          else if (opts.iframeVideo === true && typeof playPopupVideo === 'undefined')
          {
              // video player is not defined
              alert("playPopupVideo = undefined");
              $("#"+opts.popupID+" ."+opts.contentClass).append(popup.fragment);
          }
          else
          {
              $("#"+opts.popupID+" ."+opts.contentClass).append(popup.fragment);
              //console.log("regular appendage");
          };

		//$("#"+opts.popupID+" ."+opts.contentClass).append(popup.fragment);
		$("#"+opts.popupID+" ."+opts.contentClass+" "+popup.boxSrc).css("display","block");
		// code for AFL iframe video player
		//var videoTimer = setTimeout(function(){
          //    if (opts.iframeVideo === true)
		//{
          //    // get video ID number and then run the video player function
          //    if (typeof playPopupVideo !== 'undefined')
          //    {
          //        //alert("is a function");
          //        //alert(typeof playPopupVideo);
          //        playPopupVideo(popup.videoID);
          //    }
          //
          //}
          //    },20000);
	...
	$.fn.popup.checkVideo = function(popup, opts)
	{
         if (popup.frag.length > 0)
         {
             playPopupVideo(popup.videoID);
	...
	{
		$("#"+opts.popupID+" ."+opts.closeBox).click(function(e){
               e.preventDefault();
			$.fn.popup.closeBox(popup,opts);
			//return false;
		});
	...
	$.fn.popup.closeBox = function(popup,opts)
	{
         if (opts.iframeVideo == true)
		{
              document.getElementById('videoFrame').contentWindow.EmbeddedMediaPlayer.StopMedia(popup.videoID);
          }
